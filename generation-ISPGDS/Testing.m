%% update Theta_test
function [logpv_test,recErr_test,predErr_test]  =   Testing(L,K,TestData,Para,DataType,Supara,Setting)
...
Theta_test    =  cell(L,1);
L_kdott_test = cell(L,1);
L_dotkt_test = cell(L,1);
delta_test   =  cell(L,1);
Zeta_test    =  cell(L,1);
A_KT_test   =  cell(L,1);

Xt_to_t1_test = cell(L+1,1);
X_layer_split1_test = cell(L,1);
X_layer_test  = cell(L,1);

for l=1:L

        Theta_test{l}    = randg(1,K(l),Ttest);
        L_kdott_test{l} = zeros(K(l),Ttest+1);
        L_dotkt_test{l} = zeros(K(l),Ttest+1);
        delta_test{l} = ones(Ttest,1);
        Zeta_test{l}  = zeros(Ttest+1,1);

end
    %% ITER
    
 for  iter_test = 1: 1000
    
     switch DataType
            case 'Binary'
                  X_test = round(TestData);   
                [ii_test,jj_test,M_test] = find(X_test);
                iijj_test = find(X_test);
            Xmask=sparse(X_test);
            
            delta_theta = bsxfun(@times,delta_test{1}',Theta_test{1});
            Rate = Mult_Sparse(Xmask, Para.Phi{1},delta_theta);
            M_test = truncated_Poisson_rnd(full(Rate(iijj_test)));                    

             Xt_test_sparse = sparse(ii_test,jj_test,M_test,V,Ttest);                           



            case 'Count'
                 X_test = round(TestData*num_data);                       
                 Xt_test_sparse = sparse(X_test);                      
    end
    ...
    %%
            ...
            switch DataType
                    ...
                    case 'Count'
                        ...
                        %% reconstruction
                        err  =  (X_test(:,2:Ttest)-Lambda_test(:,2:Ttest))/num_data ;
                        ...
                        err = (X_test(:,2:Ttest) - Lambda_pred) / num_data ;
